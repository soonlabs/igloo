...
impl ThreadAwareAccountLocks {
    ...
    /// Returns the total number of unique accounts currently locked.
    ///
    /// This method provides a count of how many distinct accounts have any type of lock,
    /// without distinguishing between read and write locks.
    ///
    /// # Returns
    /// * `usize` - The number of unique accounts that are currently locked.
    pub(crate) fn get_locked_address(&self) -> usize {
        self.locks.len()
    }
    ...
    pub(crate) fn try_lock_accounts<'a>(
        ...
        let wl = write_account_locks.collect::<Vec<_>>().clone();
        let rl = read_account_locks.collect::<Vec<_>>().clone();

        let schedulable_threads = self
            .accounts_schedulable_threads(wl.clone().into_iter(), rl.clone().into_iter())?
            & allowed_threads;
        (!schedulable_threads.is_empty()).then(|| {
            let thread_id = thread_selector(schedulable_threads);
            self.lock_accounts(wl.clone().into_iter(), rl.clone().into_iter(), thread_id);
            thread_id
        })
    }
    ...
    pub(crate) fn schedulable_threads<const WRITE: bool>(&self, account: &Pubkey) -> ThreadSet {
