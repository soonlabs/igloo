...
use crate::{filter::filter_allows, parsed_token_accounts::*};
use jsonrpc_core::ErrorCode;
use solana_program::vote::state::VoteState;
use solana_rpc_client_api::request::{
    DELINQUENT_VALIDATOR_SLOT_DISTANCE, MAX_RPC_VOTE_ACCOUNT_INFO_EPOCH_CREDITS_HISTORY,
};
...
use {
    ...
    jsonrpc_core::{futures::future, BoxFuture, Error, Metadata, Result},
    jsonrpc_derive::rpc,
    ...
    solana_rpc_client_api::{
        ...
        request::{
            TokenAccountsFilter, MAX_GET_CONFIRMED_BLOCKS_RANGE,
...
fn is_finalized(bank: &Bank, blockstore: &Blockstore, slot: Slot) -> bool {
...
pub struct JsonRpcConfig {
    ...
    // TODO: should replace to new faucet implementation
...
/// SoonGetProofResp follows the format of eth_getProof response, but with specific soon info.
...
impl JsonRpcRequestProcessor {
    ...
    fn bank(&self, _commitment: Option<CommitmentConfig>) -> Arc<Bank> {
        // NOTE: ingore commitment, always return the working bank
    ...
    pub fn new_from_bank(bank: Bank) -> Self {
        ...
        Self {
            ...
            node_exit: create_node_exit(exit.clone()),
            genesis_hash,
    ...
    pub async fn get_inflation_reward(
        ...
        if first_slot_in_epoch < first_available_block {
            return if self.bigtable_ledger_storage.is_some() {
                Err(RpcCustomError::LongTermStorageSlotSkipped {
                    slot: first_slot_in_epoch,
                }
                .into())
            } else {
                Err(RpcCustomError::BlockCleanedUp {
                    slot: first_slot_in_epoch,
    ...
    fn get_block_commitment(&self, _block: Slot) -> RpcBlockCommitment<BlockCommitmentArray> {
        // NOTE: not supported
        RpcBlockCommitment {
            commitment: None,
            total_stake: 0,
    ...
    pub async fn get_block(
        ...
        if self.config.enable_rpc_transaction_history {
            ...
            if slot <= self.bank_forks.read().unwrap().root() {
    ...
    pub async fn get_blocks(
        ...
        let current_slot = self.get_bank_with_config(config)?.slot();
        let end_slot = min(
        ...
        let blocks: Vec<_> = self
            ...
            .filter(|&slot| slot <= end_slot && slot <= current_slot)
            .collect();
    ...
    pub async fn get_block_time(&self, slot: Slot) -> Result<Option<UnixTimestamp>> {
        ...
        if slot <= self.bank_forks.read().unwrap().root() {
            ...
            self.check_slot_cleaned_up(&result, slot)?;
            Ok(result.ok())
        } else if let Some(bank) = self.bank_forks.read().unwrap().get(slot) {
        ...
        for signature in signatures {
            let status = if let Some(status) = self.get_transaction_status(signature, &bank) {
                ...
                if let Some(status) = self
                    ...
                    .map_err(|_| Error::internal_error())?
                    .filter(|(slot, _status_meta)| *slot <= self.bank_forks.read().unwrap().root())
    ...
    pub async fn get_transaction(
        ...
        if self.config.enable_rpc_transaction_history {
            let confirmed_bank = self.bank(None);
            ...
            match confirmed_transaction.unwrap_or(None) {
                Some(mut confirmed_transaction) => {
                    ...
                    if confirmed_transaction.slot <= self.bank_forks.read().unwrap().root() {
                        return Ok(Some(encode_transaction(confirmed_transaction)?));
                    }
                }
                None => {
    ...
    pub fn get_confirmed_signatures_for_address(
        ...
        if self.config.enable_rpc_transaction_history {
            ...
            // Note: get_confirmed_signatures_for_address is deprecated and always returns an empty
            // vector
            self.blockstore
    ...
    pub async fn get_signatures_for_address(
        ...
        if self.config.enable_rpc_transaction_history {
            let highest_slot = self.get_bank_with_config(config)?.slot();
...
fn err_transfer_fn_with_loc<E: std::error::Error>(
    loc: &'static str,
) -> Box<dyn FnOnce(E) -> Error> {
    Box::new(move |error: E| Error {
        code: ErrorCode::InternalError,
        message: format!("[{}] {}", loc, error),
        data: None,
    })
}
...
fn get_encoded_account(
    ...
    match crate::account_resolver::get_account_from_overwrites_or_bank(
        pubkey,
        bank,
        overwrite_accounts,
    ) {
...
fn _send_transaction(
    ...
    // TODO: implement a transaction cache for RPC, to remove duplicated and invalid transactions
    // let transaction_info =
    //     TransactionInfo::new(transaction, last_valid_block_height, durable_nonce_info);
    meta.tx_channel
        .0
        .send(transaction)
...
pub mod rpc_minimal {
    ...
    impl Minimal for MinimalImpl {
        ...
        fn get_identity(&self, meta: Self::Metadata) -> Result<RpcIdentity> {
            Ok(RpcIdentity {
                identity: meta.bank(None).collector_id().to_string(),
            })
        }
...
pub mod rpc_bank {
    use super::*;
    use solana_rpc_client_api::request::MAX_GET_SLOT_LEADERS;
    #[rpc]
    pub trait BankData {
        type Metadata;
    ...
    impl Full for FullImpl {
        ...
        fn get_cluster_nodes(&self, meta: Self::Metadata) -> Result<Vec<RpcContactInfo>> {
            ...
            Ok(vec![RpcContactInfo {
                pubkey: meta.bank(None).collector_id().to_string(),
                gossip: None,
                tvu: None,
                tpu: None,
                tpu_quic: None,
                ...
                rpc: None,
                pubsub: None,
                version: Some(solana_version::Version::default().to_string()),
        ...
        fn request_airdrop(
            ...
            _send_transaction(meta, signature, transaction, last_valid_block_height, None)
        }
        ...
        fn send_transaction(
            ...
            let RpcSendTransactionConfig {
                ...
                encoding,
                max_retries: _, // Note: max_retries is not used here
                min_context_slot,
            } = config.unwrap_or_default();
        ...
        fn get_fee_for_message(
            ...
            let message = decode_and_deserialize::<VersionedMessage>(
                data,
...
pub fn create_node_exit(exit: Arc<AtomicBool>) -> Arc<RwLock<Exit>> {
    let mut node_exit = Exit::default();
    node_exit.register_exit(Box::new(move || exit.store(true, Ordering::Relaxed)));
    Arc::new(RwLock::new(node_exit))
}
...
#[cfg(test)]
pub(crate) fn populate_blockstore_for_tests(
    entries: Vec<Entry>,
    bank: Arc<Bank>,
    blockstore: Arc<Blockstore>,
    max_complete_transaction_status_slot: Arc<AtomicU64>,
    ...
    let transaction_status_service =
        solana_rpc::transaction_status_service::TransactionStatusService::new(
...
mod tests {
    ...
    const TEST_SIGNATURE_FEE: u64 = 5000;
    const TEST_SLOTS_PER_EPOCH: u64 = 128 + 1;

    fn create_test_request(method: &str, params: Option<Value>) -> Value {
        json!({
            "jsonrpc": "2.0",
            "id": 1u64,
            "method": method,
    ...
    impl RpcHandler {
        ...
        fn start_with_config(config: JsonRpcConfig) -> Self {
            let (bank_forks, mint_keypair, _leader_vote_keypair) =
            ...
            let exit = Arc::new(AtomicBool::new(false));
            let node_exit = create_node_exit(exit);
        ...
        fn handle_request_sync(&self, req: Value) -> Response {
            let response = &self
                .io
    ...
    fn test_rpc_simulate_transaction_with_inner_instructions() {
        ...
        let lookup_table_space = address_lookup_table::state::LOOKUP_TABLE_META_SIZE;
    ...
    fn test_rpc_send_transaction_preflight() {
        let exit = Arc::new(AtomicBool::new(false));
        let node_exit = create_node_exit(exit.clone());
        let ledger_path = get_tmp_ledger_path!();
        let blockstore = Arc::new(Blockstore::open(&ledger_path).unwrap());
    ...
    fn test_is_finalized() {
        ...
        assert!(is_finalized(&bank, &blockstore, 0));
        assert!(is_finalized(&bank, &blockstore, 1));
        assert!(!is_finalized(&bank, &blockstore, 2));
        assert!(!is_finalized(&bank, &blockstore, 3));
    }
